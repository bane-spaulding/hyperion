defmodule Hyperion.ExperimentsTest do
  ...
  describe "experiments" do
    ...
    test "create_experiment/1 with valid data creates a experiment" do
      valid_attrs = %{title: "some title", channel_id: "some channel_id", video_id: "some video_id", views: 42, thumbnail_id: 42}

      assert {:ok, %Experiment{} = experiment} = Experiments.create_experiment(valid_attrs)
      assert experiment.title == "some title"
      assert experiment.channel_id == "some channel_id"
      assert experiment.video_id == "some video_id"
      assert experiment.views == 42
      assert experiment.thumbnail_id == 42
    end
    ...
    test "update_experiment/2 with valid data updates the experiment" do
      ...
      assert {:ok, %Experiment{} = experiment} = Experiments.update_experiment(experiment, update_attrs)
      assert experiment.title == "some updated title"
      assert experiment.channel_id == "some updated channel_id"
      assert experiment.video_id == "some updated video_id"
      assert experiment.views == 43
      assert experiment.thumbnail_id == 43
    end
